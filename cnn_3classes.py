...
def cnn_por_classe(start, end):
    for test_number in range(start, end):
        ...
        for batch_size in batch_sizes:
            ...
            training_data = LungDataset(annotations_file = fr'./lung15-classes/train_{test_number}.csv', img_dir = fr'./lung15/train', transform=transform)
            test_data = LungDataset(annotations_file = fr'./lung15-classes/test_{test_number}.csv', img_dir = fr'./lung15/test', transform=transform)
            ...
            PATH = f'./models-classes/t{test_number}/pulmao_t{test_number}_{batch_size}.pth'
            ...
            print(f'Acurácia da rede neural T{test_number} com batch_size = {batch_size}: {100 * correct / total} %')

if __name__ == '__main__':
    freeze_support()
    
    # Criando duas instâncias de Process para imprimir números em paralelo
    p1 = Process(target=cnn_por_classe, args=(1, 11))
